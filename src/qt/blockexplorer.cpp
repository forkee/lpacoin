...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::LPA, nValue);
...
{
    std::string genesisblockhash = "00000fd5a71f7ab6fb21b87abe754e75b9cebf4093517447432abd095a94c71d";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (lpa.conf).");
            QMessageBox::warning(this, "LPA Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
