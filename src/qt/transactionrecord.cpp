    ...
    if (wtx.IsCoinStake()) {
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
				...
				if(nHeight <= 52560 && nHeight > 0) {
					nSubsidy = 10 * COIN;
					if(nSubsidy * 20 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 52560 && nHeight <= 525600) {
					nSubsidy = 7 * COIN;
					if(nSubsidy * 30 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 525600 && nHeight <= 1051200) { // 1051200 => LAST POW BLOCK
					nSubsidy = 5 * COIN;
					if(nSubsidy * 40 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 1051200 && nHeight <= 1576800) { // 1051200 => LAST POW BLOCK
					nSubsidy = 5 * COIN;
