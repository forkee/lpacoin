...
#if defined(NDEBUG)
#error "LPA cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "LPA Signed Message:\n";
...
{
	if (nHeight == 0) return 410000 * COIN;
	
	int64_t nSubsidy = 0;
	
	if(nHeight > 0 && nHeight <= 52560) {
        nSubsidy = 10 * COIN;
	} else if (nHeight > 52560 && nHeight <= 525600) {
		nSubsidy = 7 * COIN;
	} else if (nHeight > 525600 && nHeight <= 1576800) { // 1051200 => LAST POW BLOCK
		nSubsidy = 5 * COIN;
	} else if (nHeight > 1576800 && nHeight <= 2102400) { // 1051200 => FIRST POS BLOCK
		nSubsidy = 3 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight > 0 && nHeight <= 52560) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[LIBRA] Adjusting Libra at height %d with %d masternodes (%d % locked LPA) and %d LPA supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
